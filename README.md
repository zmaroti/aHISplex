...
The tool includes precompiled reference binary data for the GRCh37, hg19, GRCh38, and hg38 human references. With the '-r' option this can be specified at step 1. Obviously different reference alignments cannot be mixed in the same analysis as the marker positions are different in hg38 and hg19. Furthemore, also **NOTE** that GLIMPSE2_phase cannot deal with the different naming conventions of GRCH37/hg19 (N vs chrN) even though the genome positions are identical in these references. Accordingly, alignments (BAM files) with different references or naming conventions have to be analyzed separately. The hg38 and GRCh38 reference data has same naming convention (for the autsomal regions containing the HIrisPlex-S markers) and also the marker genom positions are identical, hence alignments to these references can be analyzed together.
```sh
aHISplex.sh -r hg38 hg38_aligned_BAM_file.bam
```

## optimizing parallel analysis
In case GNU parallel is installed, the software tries to use all available CPU cores to parallelly run the imputation of the 11 genome regions. The number of cores can be overridden by the '-t' option of aHISplex.sh. Since GLIMPSE2_phase can also use more threads to impute a chunk of genome region, it is recommended to specify core numbers that are divisible by 11 (the number of genome chunks harboring the 41 HirisPlex-S system markers). You can provide higher than available physical or virtual CPU cores, as the Linux OS will then split the available system resources between threads. For example, on a 20-virtual-core machine, you can specify 22 cores (*-t 22*) for optimal speed analysis, allowing parallel imputation of the 11 genome regions with 2 threads allocated to each GLIMPSE2_phase process. However, it is not recommended to significantly overcommit the available CPU cores, as it may lead to too much content switching between tasks and underwhelming performance. Since the imputation is only performed on the regions harbouring the 41 HIrisPlex-S markers (with a 5mb flanking genomic context), the analysis should be relatively quick compared to whole-genome imputations.
